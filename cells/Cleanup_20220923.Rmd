...
```{r, message = F}
list.files()
mydata <- read_csv("PFA_pos_height.csv")
...
a <- as.name(colnames(select(Y,matches("IS"))))
print(a)
# Copy this into the second part of the equation below. Make sure it is the PB

Y1 <- mutate(Y, sub_height = `Height (avg)` - `[Height] IS_4X_pos_091622_032`)
# Set NA values to 0
...
for (j in 1:nrow(Y1)){
   for (i in 4:16) {
    if(is.na(Y1[j, 3]) == F && is.na(Y1[j, i]) == F && Y1[j, 3] >= Y1[j,i]){
      Y1[j,]$PB_comp = Y1[j,]$PB_comp + 1
    }else if(is.na(Y1[j, 3]) == F && is.na(Y1[j, i]) == T){
      Y1[j,]$PB_comp = Y1[j,]$PB_comp + 1
...
```{r}
colnames(Y3)
colnames(Y3) <- gsub(".*sample_", "", colnames(Y3))
colnames(Y3) <- gsub("_4X.*" , "", colnames(Y3))
print(colnames(Y3))
```
...
rm_lipid <- lipidup.list[] 
rm_lipid_2 <- lipidup.list_2[c(1:11)]
...
```{r}
list.files()
sample_info <- read_csv("hepa16_samples.csv")
...
```{r}
write_csv(lipid_data, "cleanup/pos_height_initial.csv")
rm(list = ls())
```
...
neg_dat <- read_csv("cleanup/neg_height_initial.csv")
...
pos_IS <- append(pos_IS, colnames(select(pos_dat , contains("18:1(d3)")))[1])
pos_IS <- sort(pos_IS)
...
Set the dilution factor (DF) and volume multiplier here to make negative and positive mode indicative of the same injected sample amount. Samples resuspended in 75 uL of MeOH. Volume multiplier is 7 uL injected in negative mode of undiluted, and 5 uL injected in positive mode if 4X diluted. 

10 uL of each standard injected

Concentrations from Avanti SPLASH II mix Lot #330709W, MLOT 6453WAA010
AC standard 1mg/mL
...
PC_p_pmol <- ((158.2/753.11)*0.01*(5/75)*1000*1000*(1/4))
LPC_p <- pos_dat$`LPC 18:1/0:0(d7)`
LPC_p_pmol <- ((23.8/538.72)*0.01*(5/75)*1000*1000*(1/4))
PE_p <- pos_dat$`PE 15:0_18:1(d7)`
PE_p_pmol <- ((5.0/711.03)*0.01*(5/75)*1000*1000*(1/4))
PS_p <- pos_dat$`PS 15:0_18:1(d7)`
PS_p_pmol <- ((7.8/777.02)*0.01*(5/75)*1000*1000*(1/4))
TG_p <- pos_dat$`TG 15:0_18:1(d7)_15:0`
TG_p_pmol <- ((56.9/812.37)*0.01*(5/75)*1000*1000*(1/4))
SM_p <- pos_dat$`SM d18:1_18:1(d9)`
SM_p_pmol <- ((29.5/738.12)*0.01*(5/75)*1000*1000*(1/4))
CE_p <- pos_dat$`CE 18:1(d7)`
CE_p_pmol <- ((348.8 /658.16)*0.01*(5/75)*1000*1000*(1/4))
AC_p <- pos_dat$`ACar 18:1(d3)`
AC_p_pmol <- ((1000/465.1)*0.01*(5/75)*1000*1000*(1/4))
...
pos_dat[,grepl("^PS" , names(pos_dat))] <- 
  (pos_dat[,grepl("^PS" , names(pos_dat))]/PC_p)*PS_p_pmol
```

```{r}
pos_dat[,grepl("^ACar" , names(pos_dat))] <- 
  (pos_dat[,grepl("^ACar" , names(pos_dat))]/LPC_p)*AC_p_pmol
...
pos_dat <- pos_dat[,!grepl(c("CL"),colnames(pos_dat))]
```
...
LPC_n_pmol <- ((23.8/528.72)*0.01*(7/75)*1000*1000)
LPE_n <- neg_dat$`LPE 18:1(d7)`
LPE_n_pmol <- ((0.5/486.64)*0.01*(7/75)*1000*1000)
PC_n <- neg_dat$`PC 15:0_18:1(d7)`
PC_n_pmol <- ((158.2/753.11)*0.01*(7/75)*1000*1000)
PE_n <- neg_dat$`PE 15:0_18:1(d7)`
PE_n_pmol <- ((5.0/711.03)*0.01*(7/75)*1000*1000)
PI_n <- neg_dat$`PI 15:0_18:1(d7)`
PI_n_pmol <- ((8.5/847.13)*0.01*(7/75)*1000*1000)
PS_n <- neg_dat$`PS 15:0_18:1(d7)`
PS_n_pmol <- ((7.8/777.02)*0.01*(7/75)*1000*1000)
...
neg_dat[,grepl("^EtherOxPC", names(neg_dat))] <- 
  (neg_dat[,grepl("^EtherOxPC", names(neg_dat))]/PE_n)*PC_n_pmol
neg_dat[,grepl("^EtherPC", names(neg_dat))] <- 
  (neg_dat[,grepl("^EtherPC", names(neg_dat))]/PE_n)*PC_n_pmol
...
neg_dat[,grepl("^PC", names(neg_dat))] <- 
  (neg_dat[,grepl("^PC", names(neg_dat))]/PE_n)*PE_n_pmol
...
neg_dat <- neg_dat[,!grepl(c("Cer"),colnames(neg_dat))]
neg_dat <- neg_dat[,!grepl(c("SM"),colnames(neg_dat))]
neg_dat <- neg_dat[,!grepl(c("GM3"),colnames(neg_dat))]
neg_dat <- neg_dat[,!grepl(c("PG"),colnames(neg_dat))]
neg_dat <- neg_dat[,!grepl(c("CL"),colnames(neg_dat))]
```
...
shared_test <- intersect(colnames(neg_final)[5:ncol(neg_final)], 
                    colnames(pos_final[5:ncol(pos_final)]))
print(shared_test)
...
# Normalize to starting protein amount (measured by BCA)
```{r}
IS_rm <- colnames(select(Lipids_Final, matches("(d7)")))
IS_rm  <- append(IS_rm, colnames(select(Lipids_Final, matches("(d9)"))))
IS_rm  <- append(IS_rm, colnames(select(Lipids_Final, contains("ACar 18:1(d3)"))))
Lipids_Final <- Lipids_Final[,!names(Lipids_Final) %in% IS_rm]
Lipids_Final_norm <- Lipids_Final
Lipids_Final_norm[,5:ncol(Lipids_Final_norm)] <- Lipids_Final[,5:ncol(Lipids_Final)]/Lipids_Final$`protein (mg)`
...
write_csv(Lipids_Final_norm, "cleanup/combined_pmol_mg_protein.csv")
rm(list=ls())
```
