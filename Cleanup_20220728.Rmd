...
```{r, message = F}
list.files()
mydata <- read_csv("PFA_2021.02_neg_height.csv")
...
Y1 <- mutate(Y, sub_height = `Height (avg)` - `[Height] liver_PB_neg_02172021_116`)
# Set NA values to 0
...
rm_lipid <- lipidup.list[c(1)] 
rm_lipid_2 <- lipidup.list_2[c(2:3)]
...
```{r}
write_csv(lipid_data, "cleanup/neg_height_initial.csv")
rm(list = ls())
```
...
neg_dat[,grepl("^EtherPC", names(neg_dat))] <- 
  (neg_dat[,grepl("^EtherPC", names(neg_dat))]/PE_n)*PE_n_pmol
...
write_csv(Lipids_Final_norm, "cleanup/combined_pmol_mg_tissue.csv")
rm(list=ls())
```
